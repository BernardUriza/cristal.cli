...
namespace Cristal.CLI.Labyrinth.Dream
{
    ...
    public class DreamTunnelSystem : MonoBehaviour
    {
        ...
        [SerializeField] private bool _useAIGeneration = true;
        [SerializeField] private bool _projectSymbols = true;
        [SerializeField] private int _symbolsPerRoom = 2;

        [Header("Room Definitions")]
        [SerializeField] private DreamRoomDefinition[] _roomDefinitions;
        [SerializeField] private DreamRoomDefinition _unboundRoomDefinition;
        ...
        public void CreateDreamFromArcana(ArcanaDefinition arcana)
        {
            ...
            var roomDef = FindRoomDefinitionForArcana(arcana.id);
            ...
            // Apply room definition if found
            if (roomDef != null)
            {
                ApplyRoomDefinitionToConfig(config, roomDef);
            }
            ...
            if (_useAIGeneration)
            {
                StartCoroutine(GenerateAIContentForTunnel(tunnel, arcana));
            }

            // Project symbols
            if (_projectSymbols)
            {
                StartCoroutine(ProjectSymbolsInTunnel(tunnel, arcana.id));
            }

            // Record in memory
            var memoryBridge = DreamMemoryBridge.Instance;
            if (memoryBridge != null)
            {
                var context = memoryBridge.BuildDreamContext(config.themeName);
                memoryBridge.RecordDreamEntry(config.themeName, context);
            }
        ...
        private DreamRoomDefinition FindRoomDefinitionForArcana(int arcanaId)
        {
            if (_roomDefinitions == null) return null;

            foreach (var def in _roomDefinitions)
            {
                if (def != null && def.CanTriggerByArcana(arcanaId))
                {
                    return def;
                }
            }
        ...
        private DreamRoomDefinition FindRoomDefinitionForEmotion(string emotion)
        {
            if (_roomDefinitions == null) return null;

            foreach (var def in _roomDefinitions)
            {
                if (def != null && def.CanTriggerByEmotion(emotion))
                {
                    return def;
                }
            }
        ...
        private void ApplyRoomDefinitionToConfig(DreamConfig config, DreamRoomDefinition def)
        {
            config.primaryColor = def.primaryColor;
            config.secondaryColor = def.secondaryColor;
            config.fogDensity = def.fogDensity;
            config.distortionIntensity = def.glitchIntensity;
            config.symbolism = def.symbolism;
            config.roomCount = def.segmentCount;
            config.duration = UnityEngine.Random.Range(def.minDuration, def.maxDuration);

            if (def.fallbackNarratives != null && def.fallbackNarratives.Length > 0)
            {
                config.narrativeFragments = new List<string>(def.fallbackNarratives);
            }
        }
        ...
        private IEnumerator GenerateAIContentForTunnel(DreamTunnel tunnel, ArcanaDefinition arcana)
        {
            var oracle = DreamAIOracle.Instance;
            if (oracle == null) yield break;

            var memoryBridge = DreamMemoryBridge.Instance;
            var context = memoryBridge?.BuildDreamContext(tunnel.Config.themeName) ?? new DreamContext();

            if (arcana != null)
            {
                context.ActiveArcana = arcana;
                context.DreamTheme = arcana.name;
            }

            // Generate room name
            string generatedName = null;
            yield return oracle.GenerateContentAsync(
            ...
            {
                tunnel.Config.themeName = generatedName;
                if (_debugMode)
                {
                    Debug.Log($"[DreamTunnelSystem] AI generated name: {generatedName}");
                }
            }

            // Generate wall inscriptions
            foreach (var room in tunnel.Rooms)
            {
                ...
                {
                    room.AddWallInscription(inscription);
                    memoryBridge?.RecordInscription(inscription);

                    if (_debugMode)
                    {
                        Debug.Log($"[DreamTunnelSystem] AI inscription: {inscription}");
                    }
                }
            ...
            {
                tunnel.AddNarrativeFragment(new DreamNarrativeFragment
                {
                    text = narrative,
                    displayType = NarrativeDisplayType.FloatingText,
                    textColor = tunnel.Config.primaryColor,
                    isAIGenerated = true
                });
            }
        }

        private IEnumerator ProjectSymbolsInTunnel(DreamTunnel tunnel, int arcanaId)
        {
            var projector = DreamSymbolProjector.Instance;
            if (projector == null) yield break;

            // Wait for tunnel to be active
            yield return new WaitForSeconds(1f);

            var symbolDef = SymbolDefinition.FromArcana(arcanaId);
            symbolDef.color = tunnel.Config.primaryColor;
            symbolDef.glowColor = tunnel.Config.secondaryColor;

            foreach (var room in tunnel.Rooms)
            {
                var bounds = room.GetRoomBounds();

                for (int i = 0; i < _symbolsPerRoom; i++)
                {
                    var symbol = projector.ProjectOnRandomWall(symbolDef, bounds);

                    if (symbol != null)
                    {
                        var memoryBridge = DreamMemoryBridge.Instance;
                        memoryBridge?.RecordSymbolEncounter(symbolDef.type.ToString());

                        if (_debugMode)
                        {
                            Debug.Log($"[DreamTunnelSystem] Projected symbol: {symbolDef.type}");
                        }
                    }
